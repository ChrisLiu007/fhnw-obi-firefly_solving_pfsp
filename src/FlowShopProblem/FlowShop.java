 ...
 *  Contributor(s):
 *  @author Joël Schmid, enhanced output and restructuring
 *
 * ################################################################
...
/**
 * FlowShop describe a problem instance with this jobs and the number of machine
 * and k
 * 
...
public class FlowShop implements Serializable {
	...
	public FlowShop(int nbMachine, int[][] jobs) {
		...
		for (int i = 0; i < jobs.length; i++) {
			...
			// JS: additional output
			// System.out.println("cummulate Time on last machine:" +
			// cumulateTimeOnLastMachine);
		}
	 ...
	 * @param fs
	 * @param permutation
	 * @return the makespan, the overall completion time for all the jobs of the
	 *         fs
	 */
	 ...
	 * @param permutation
	 * @param nbJob
	 *            the length use of the permutation, begin 0
	 ...
	 * @param nbJob
	 * @param bound
	 * @return the makespan if it not exceed bound else the negative index where
	 *         the makespan exeed the bound
	 */
	...
	public static long computeLowerBound(FlowShop fs) {
		...
		for (int j = 0; j < lastMachineId; j++) { // we don't add the last
													// machine time, already in
													// timeOnLastMachines
		...
		for (int i = 1; i < fs.jobs.length; i++) {
			...
			for (int j = 0; j < lastMachineId; j++) { // we don't add the last
														// machine time, already
														// in timeOnLastMachines
